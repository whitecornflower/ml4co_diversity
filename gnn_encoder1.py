...
from diffusion.models.nn import (
    linear,
    zero_module,
    normalization,
    timestep_embedding,
...
def run_sparse_ffn_layer(ffn_layer, ffn_time_layer, ffn_out_layer, add_time_on_edge=True):
    def custom_forward(*inputs):
        x_in = inputs[0]
        e_in = inputs[1]
        time_emb = inputs[2]
        ...
        if add_time_on_edge:
            # Apply FFN to edge features
            e = ffn_layer(e_in)
            e = e + ffn_time_layer(time_emb)
            e = e_in + ffn_out_layer(e)
            x = x_in
        else:
            # Apply FFN to node features
            x = ffn_layer(x_in)
            x = x + ffn_time_layer(time_emb)
            x = x_in + ffn_out_layer(x)
            e = e_in
...
class GNNEncoder(nn.Module):
  ...
  """
    ...
    def __init__(self, n_layers, hidden_dim, out_channels=1, aggregation="sum", norm="layer",
                 ...
                 add_ffn=True, ffn_expansion=8, ffn_dropout=0.1,
                 *args, **kwargs):
        ...
        # Add FFN layer between n_layers-2 and n_layers-1 
        if self.add_ffn and n_layers >= 3:
            ...
            self.ffn_time_layer = nn.Sequential(
                nn.ReLU(),
                linear(time_embed_dim, hidden_dim),
            )
            
            self.ffn_out_layer = nn.Sequential(
                nn.LayerNorm(hidden_dim, elementwise_affine=learn_norm),
                nn.SiLU(),
    ...
    def apply_ffn_layer(self, x, e, time_emb, sparse=False):
        """Apply FFN layer"""
        if not self.add_ffn:
            return x, e
        ...
        if not self.node_feature_only:
            ...
            if sparse:
                e_in = e
                e = self.ffn_layer(e)
                e = e + self.ffn_time_layer(time_emb)
                e = e_in + self.ffn_out_layer(e)
            else:
                batch_size, num_nodes, _, hidden_dim = e.shape
                e_in = e
                e_flat = e.view(-1, hidden_dim)
                e_flat = self.ffn_layer(e_flat)
                e = e_flat.view(batch_size, num_nodes, num_nodes, hidden_dim)
                e = e + self.ffn_time_layer(time_emb)[:, None, None, :]
                e = e_in + self.ffn_out_layer(e.view(-1, hidden_dim)).view(batch_size, num_nodes, num_nodes, hidden_dim)
        else:
            # Apply FFN to node features
            x_in = x
            if sparse:
                x = self.ffn_layer(x)
                x = x + self.ffn_time_layer(time_emb)
            else:
                batch_size, num_nodes, hidden_dim = x.shape
                x_flat = x.view(-1, hidden_dim)
                x_flat = self.ffn_layer(x_flat)
                x = x_flat.view(batch_size, num_nodes, hidden_dim)
                x = x + self.ffn_time_layer(time_emb)[:, None, :]
            x = x_in + self.ffn_out_layer(x)
    ...
    """
        ...
        for i, (layer, time_layer, out_layer) in enumerate(zip(self.layers, self.time_embed_layers, self.per_layer_out)):
            x_in, e_in = x, e
            ...
            # Apply FFN layer after n_layers-3
            if self.add_ffn and i == self.n_layers - 3:
                x, e = self.apply_ffn_layer(x, e, time_emb, sparse=False)
    ...
    def sparse_encoding(self, x, e, edge_index, time_emb):
        ...
        for i, (layer, time_layer, out_layer) in enumerate(zip(self.layers, self.time_embed_layers, self.per_layer_out)):
            ...
            if self.use_activation_checkpoint:
                ...
                # Apply FFN with activation checkpointing
                if self.add_ffn and i == self.n_layers - 3:
                    run_ffn_layer_fn = functools.partial(
                        run_sparse_ffn_layer,
                        add_time_on_edge=not self.node_feature_only
                    )
                    
                    out = activation_checkpoint.checkpoint(
                        run_ffn_layer_fn(self.ffn_layer, self.ffn_time_layer, self.ffn_out_layer),
                        x, e, single_time_emb
                    )
                    x = out[0]
                    e = out[1]
                ...
                # Apply FFN layer after n_layers-3
                if self.add_ffn and i == self.n_layers - 3:
                    x, e = self.apply_ffn_layer(x, e, time_emb, sparse=True)
